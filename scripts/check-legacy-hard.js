...
const PATTERNS = [
  ...
  { regex: /<v-list-item-content\b/g, desc: 'Vuetify 2 component <v-list-item-content>' },
  { regex: /<v-list-item-icon\b/g, desc: 'Vuetify 2 component <v-list-item-icon>' },
  { regex: /<v-list-item-avatar\b/g, desc: 'Vuetify 2 component <v-list-item-avatar>' },
  { regex: /<v-list-item-group\b/g, desc: 'Vuetify 2 component <v-list-item-group>' },
  ...
  { regex: /\b:?item-text\s*=/g, desc: 'Vuetify 2 item-text prop (use item-title)' },
  { regex: /\$emit\s*\(\s*["']input["']/g, desc: "Vue 2 custom v-model event ($emit('input'))" },
...
function reportLegacyPattern(desc, relPath, lineNumber, line) {
  console.log(`Found hard legacy pattern: ${desc}`);
...
console.log('Checking hard Vue 2 / Vuetify 2 migration blockers...');
  ...
  walkDir(fullScanPath, (filePath, relPath) => {
    ...
    for (const pattern of PATTERNS) {
      ...
      while ((match = pattern.regex.exec(content)) !== null) {
        ...
        reportLegacyPattern(pattern.desc, relPath, lineNumber, line);
      }
    }
  });
}
...
if (matchCount > 0) {
  console.log(`Hard legacy check failed. Found ${matchCount} matches.`);
  process.exit(1);
}

console.log('Hard legacy check passed.');
