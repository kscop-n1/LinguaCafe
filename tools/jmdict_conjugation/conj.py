...
def main():
    ...
    if pos not in [x[0] for x in ct['conjo']]:
        sys.exit(
            'no conjugation data available for part-of-speech: %s\n'
            "'conj.py --list' will "
            'print a list of conjugatable parts-of-speech' % args.pos
        )
    ...
    for conj, conjnm in sorted(ct['conj'].values(), key=lambda x: x[0]):
        for neg, fml in (0, 0), (0, 1), (1, 0), (1, 1):
            ...
            for onum in range(1, 10):  # onum values start at 1, not 0.
                try:
                    _, _, _, _, _, stem, okuri, euphr, euphk, _ = ct['conjo'][
                        pos, conj, neg, fml, onum
                    ]
                except KeyError:
                    break
...
def construct(txt, stem, okuri, euphr, euphk):
    ...
    if len(txt) < 2:
        raise ValueError('Conjugatable words must be at least 2 characters long')
    iskana = txt[-2] > 'あ' and txt[-2] <= 'ん'
    if iskana and euphr or not iskana and euphk:
        stem += 1
    if iskana:
        conjtxt = txt[:-stem] + (euphr or '') + okuri
    else:
        conjtxt = txt[:-stem] + (euphk or '') + okuri
    return conjtxt
...
def parse_args(argv=None):
    ...
    p.add_argument('--help', action='help', help='Print this help message.')
