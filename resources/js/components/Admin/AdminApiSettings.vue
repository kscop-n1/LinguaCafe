		...
		<v-form v-model="isFormValid">
			...
			<v-card outlined class="rounded-lg pa-4 pt-0" :loading="characterLimitLoading">
				<v-card-text id="deepl-card-text">
					<!-- DeepL cache -->
					<label class="font-weight-bold"> DeepL Cache </label>
					...
					>
						...
						<v-card-text class="pa-0" v-if="!characterLimitLoading">
							...
							{{ formatNumber(characterLimit - characterUsed).replace('&nbsp;', '') }}
							characters left
						</v-card-text>
					</v-card>
			...
			axios
				...
				.then(response => {
					...
					if (response.status === 200) {
						...
						this.cachedDeeplTranslations = response.data.data.cachedDeeplTranslations
						this.characterUsed = response.data.data.limits.character_count ?? null
						this.characterLimit = response.data.data.limits.character_limit ?? null
