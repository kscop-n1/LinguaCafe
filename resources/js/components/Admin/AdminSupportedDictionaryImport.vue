<template>
	...
	>
					...
					<v-stepper-content step="3">
						...
						<template v-if="!importing && importResult !== null">
							...
							>
								...
								{{ dictionary.name }} has been imported.
							</v-alert>
		...
		<v-card-actions>
			...
			<template v-if="!importing && importResult === null">
				<v-btn rounded text @click="back">Back</v-btn>
				<v-btn
					v-if="
						stepperPage == 2 &&
						importResult === null &&
...
export default {
	...
	methods: {
		...
		startImport() {
			this.$store.getters['shared/echo']
				.private('dictionary-import-progress.' + this.$store.getters['shared/userUuid'])
				.listen('DictionaryImportProgressedEvent', message => {
					this.importedRecords = message.importedRecords

					// update percentage
					this.importingProgressPercentage = parseInt(
						(this.importedRecords / this.dictionary.expectedRecordCount) * 100
					)
