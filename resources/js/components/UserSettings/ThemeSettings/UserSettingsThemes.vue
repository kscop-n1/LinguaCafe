<template>
    <div id="user-settings-themes">
        ...
        <user-settings-text-styling :selected-theme="selectedTheme" @update="updateTextStyling" />

        <!-- Color header -->
        <div class="subheader mt-4 d-flex">UI colors</div>

        <!-- Color table -->
        <v-card v-if="selectedTheme === 'eink'" outlined class="rounded-lg mt-2" :loading="loading">
            <v-card-text> E-ink UI colors are not modifiable. </v-card-text>
        </v-card>
        ...
        <v-form v-else v-model="isFormValid">
            <v-simple-table class="rounded-lg no-hover border mt-2" v-if="!loading">
                ...
                <tbody>
                    <tr
                        v-for="(color, index) in selectedTheme == 'light' ? lightTheme : darkTheme"
                        :key="selectedTheme + color.name"
                    >
                        <td>
                            {{ color.name }}
                        ...
                        <td>
                            <v-btn icon title="Restore default" @click="resetColor(index)">
...
export default {
    data: function () {
        return {
            ...
            textStyling: null,
            selectedTheme:
                ThemeService.getCurrentTheme() === 'auto'
                    ? ThemeService.getAutoTheme()
            ...
            colorInformations: {
                newWordBackground:
                    'Used as background for indicating or displaying a new word outside of the interactive text areas.',
                highlightedWordBackground:
                    'Used as background for indicating or displaying a highlighted word outside of the interactive text areas.',
                highlightedWordText:
                    'Used as text color for indicating or displaying a highlighted word outside of the interactive text areas.',
            },
