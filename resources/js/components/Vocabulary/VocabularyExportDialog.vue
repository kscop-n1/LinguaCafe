<template>
	<v-dialog v-model="value" persistent scrollable width="900px">
		<v-card id="vocabulary-export-dialog" class="rounded-lg">
			<!-- Title bar -->
			<v-card-title>
			...
			<v-card-text>
				...
				<v-simple-table
					fixed-header
					id="vocabulary-export-sample-table"
				...
				>
					...
					<tbody>
						<tr v-for="(sampleWord, wordIndex) in $props.sampleWords.slice(0, 10)" :key="wordIndex">
							...
							<td class="default-font" v-if="fields.lemmaReading">
								{{ sampleWord.lemma_reading }}
							</td>
							<td class="default-font" v-if="fields.reading">
								{{ sampleWord.reading }}
							</td>
