                ...
                <v-row id="filters" :class="{ hidden: filtersHidden }">
                    ...
                    <v-menu offset-y>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn
                                class="filter-menu pl-3 pr-2 mx-1"
                                color="foreground"
                                rounded
                                depressed
                                v-bind="attrs"
                        ...
                        <v-list class="filter-popup pa-0" dense>
                            <v-list-item-group color="primary">
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == -999 }"
                                    @click="applyFilter('stage', -999)"
                                    >Any</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == 2 }"
                                    @click="applyFilter('stage', 2)"
                                    >New</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == 1 }"
                                    @click="applyFilter('stage', 1)"
                                    >Ignored</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == 0 }"
                                    @click="applyFilter('stage', 0)"
                                    >Learned</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == -1 }"
                                ...
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == -3 }"
                                    @click="applyFilter('stage', -3)"
                                    >3</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == -4 }"
                                    @click="applyFilter('stage', -4)"
                                    >4</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == -5 }"
                                    @click="applyFilter('stage', -5)"
                                    >5</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == -6 }"
                                    @click="applyFilter('stage', -6)"
                                    >6</v-list-item
                                >
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.stage == -7 }"
                                    @click="applyFilter('stage', -7)"
                                    >7</v-list-item
                                >
                            </v-list-item-group>
                        </v-list>
                    ...
                    <v-menu right offset-y v-if="books.length">
                        ...
                        <v-list class="filter-popup pa-0" dense>
                            <v-list-item-group color="primary">
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.book == -1 }"
                                    @click="applyFilter('book', -1, -1)"
                                    >Any</v-list-item
                                >
                                <v-list-item
                                    ...
                                    :class="{
                                        'default-font': true,
                                        'v-list-item--active': filters.book == book.id,
                                    }"
                                    @click="applyFilter('book', book.id, index)"
                                    >{{ book.name }}
                                </v-list-item>
                            </v-list-item-group>
                        </v-list>
                    ...
                    <v-menu offset-y v-if="filters.bookIndex !== -1 && books.length">
                        ...
                        <v-list class="filter-popup pa-0" dense>
                            <v-list-item-group color="primary">
                                <v-list-item
                                    :class="{ 'v-list-item--active': filters.chapter == -1 }"
                                    @click="applyFilter('chapter', -1)"
                                    >Any</v-list-item
                                >
                                <v-list-item
                                    v-for="(chapter, index) in books[filters.bookIndex].chapters"
                                    :key="index"
                                    :class="{
                                        'default-font': true,
                                        'v-list-item--active': filters.chapter == chapter.id,
                                    }"
                                    @click="applyFilter('chapter', chapter.id, index)"
                                    >{{ chapter.name }}
                                </v-list-item>
                            </v-list-item-group>
                        </v-list>
                        ...
                        <v-list class="filter-popup pa-0" dense>
                            <v-list-item-group color="primary">
                                <v-list-item
                                    ...
                                    @click="applyFilter('translation', 'any')"
                                    >Any
                                </v-list-item>
                                <v-list-item
                                    :class="{
                                        'v-list-item--active': filters.translation == 'not empty',
                                    }"
                                    @click="applyFilter('translation', 'not empty')"
                                    >Not empty
                                </v-list-item>
                            </v-list-item-group>
                        </v-list>
                        ...
                        <v-list class="filter-popup pa-0" dense>
                            <v-list-item-group color="primary">
                                <v-list-item
                                    ...
                                    @click="applyFilter('phrases', 'both')"
                                    >Both
                                </v-list-item>
                                <v-list-item
                                    :class="{
                                        'v-list-item--active': filters.phrases == 'only words',
                                    }"
                                    @click="applyFilter('phrases', 'only words')"
                                    >Only words
                                </v-list-item>
                                <v-list-item
                                    :class="{
                                        'v-list-item--active': filters.phrases == 'only phrases',
                                    }"
                                    @click="applyFilter('phrases', 'only phrases')"
                                    >Only phrases
                                </v-list-item>
                            </v-list-item-group>
                        </v-list>
                    ...
                    <!-- Show filters -->
                    <v-btn
                        class="filter-menu show-filters px-3"
                        rounded
                        depressed
                    ...
                    <v-menu offset-y>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn
                                class="filter-menu export pl-3 pr-2"
                                color="foreground"
                                rounded
                                depressed
                                v-bind="attrs"
            ...
            <tbody>
                <tr v-for="(word, index) in words" :key="index">
                    <template v-if="word.type == 'phrase'">
                        <td class="word default-font" v-if="!languageSpaces">
