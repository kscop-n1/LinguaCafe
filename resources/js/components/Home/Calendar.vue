	...
	<div>
		<div class="subheader subheader-margin-top d-flex">
			...
			<v-menu offset-y class="rounded-lg">
				...
				<v-btn
					class="menu-button justife-start"
					tile
					color="white"
					@click="
			...
			>
				<template v-slot:activator="{ on, attrs }">
					...
					>
						<span id="calendar-date-button-text">{{ pickerDateFormated }}&nbsp;</span
				...
				<div class="calendar-month" v-for="(month, index) in selectedMonths" :key="index">
					...
					<div class="calendar-month-days">
						<div
							...
							:key="index + '-' + dayIndex"
						>
							<div class="calendar-day-background"></div>
...
export default {
	...
	methods: {
		...
		openCalendarDayPopup(event, day) {
			...
			for (var i = 0; i < this.calendarData.length; i++) {
				if (this.calendarData[i].day == this.popupMenu.day.fullDate) {
					this.popupMenu.achievements = JSON.parse(
						JSON.stringify(this.calendarData[i].achievements)
					)
					break
				}
			}
			...
			this.popupMenu.achievements.sort(
				(a, b) => defaultGoalTypes.indexOf(a.type) - defaultGoalTypes.indexOf(b.type)
			)
		...
		updateCalendar() {
			...
			var currentDate = new moment(this.currentMonth.format('YYYY-MM-DD'))
				.startOf('month')
				.subtract(4, 'months')
			...
			while (currentDate.isBefore(lastDay)) {
				if (
					...
					this.selectedMonths[this.selectedMonths.length - 1].formattedString !==
