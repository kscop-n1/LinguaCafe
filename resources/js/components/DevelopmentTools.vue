<template>
	<div>
		<v-card outlined class="mx-auto my-8 pa-6 rounded-xl" width="1500px">
			...
			<v-radio-group v-model="method" class="mt-0" @change="save">
			...
			<v-card-actions>
				<v-btn rounded depressed color="primary" @click="makeRequest">Make request</v-btn>
			</v-card-actions>
		</v-card>
		...
		>
			<template v-if="!loading">
				<v-label class="font-text-weight pt-6">Status: {{ responseStatus }}</v-label
				><br />

				<v-label class="font-text-weight mt-4">Response:</v-label>
				<pre
					class="text--text rounded-xl pa-6 mt-2"
					style="background-color: var(--v-gray3-base)"
				><!--
                    -->{{ responseData }}<!--
                --></pre>
			</template>
