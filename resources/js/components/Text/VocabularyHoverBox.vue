<template>
    ...
    >
        ...
        <ul class="mb-0 pl-0">
            <!-- User translations -->
            <li
                v-for="(translation, translationIndex) in userTranslationList"
                :key="'user-' + translationIndex"
            >
            ...
            <template
                v-if="
                    !['loading', 'dictionary-search-disabled'].includes(dictionaryTranslation) &&
                    dictionaryTranslationList.length
                "
            >
                <li
                    v-for="(translation, translationIndex) in dictionaryTranslationList"
                    :key="'dictionary-' + translationIndex"
                >
            ...
            <template
                v-if="
                    ...
                    apiTranslations[0] !== 'loading' &&
                    apiTranslations[0] !== 'error'
                "
            >
                <li
                    key="api-translation"
                    v-for="(translation, index) in apiTranslations"
                    :key="'api-' + index"
                >
...
export default {
    ...
    mounted() {
        ...
        if (this.$store.state.hoverVocabularyBox.dictionaryTranslation.length) {
            this.dictionaryTranslationList =
                this.$store.state.hoverVocabularyBox.dictionaryTranslation.split(';')
        }
