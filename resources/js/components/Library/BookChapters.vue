            ...
            <template v-slot:item.actions="{ item }">
                <div class="d-flex justify-center">
                    ...
                    <template v-if="item.processing_status == 'processed'">
                        <v-btn icon :to="'/chapters/read/' + item.id" title="Read"
    ...
    methods: {
        chapterStatusUpdate(chapters) {
            this.chapters.forEach(currentChapter => {
                ...
                if (
                    'wordCount' in chapters[currentChapter.id] &&
                    chapters[currentChapter.id].wordCount !== null
                ) {
                ...
                .then(
                    axios.spread((bookResponse, chaptersResponse) => {
                        ...
                        if (this.chapters.length) {
                            this.randomChapter =
                                this.chapters[Math.floor(Math.random() * this.chapters.length)].id
                        } else {
