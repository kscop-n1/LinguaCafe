<template>
	<div class="d-flex flex-column justify-center flex-nowrap">
		...
		<v-alert
			...
			dark
		>
			Choose a subtitle from the media currently being played on your Jellyfin server. You can
			import them seamlessly into LinguaCafe for later reading.
		</v-alert>
		...
		>
			...
			<div
				class="regular-list-height subtitle header rounded-pill my-2"
				v-if="!$props.subtitleLoading"
			>
			...
			<template v-if="subtitleListError">
				<v-alert color="error" type="error" border="left" dark>
			...
			<template
				...
				v-if="!subtitleListLoading && !$props.subtitleLoading && sessions.length"
			>
				<div
					class="regular-list-height subtitle rounded-pill my-2"
				...
				>
					...
					<div class="subtitle-media" v-if="session.type == 'Episode'">
						{{ session.seriesName }} S{{ ('0' + session.seriesSeason).slice(-2) }}E{{
							('0' + session.seriesEpisode).slice(-2)
						}}
						- {{ session.title }}
					</div>
			...
			axios
				...
				.then(result => {
					...
					for (let sessionIndex = 0; sessionIndex < sessions.length; sessionIndex++) {
						for (
							let subtitleIndex = sessions[sessionIndex].subtitles.length - 1;
							subtitleIndex >= 0;
							subtitleIndex--
						) {
