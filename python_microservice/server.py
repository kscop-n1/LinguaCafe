...
def tokenizeSubtitles():
    ...
    return json.dumps(
        tokenizerService.tokenizeSubtitles(subtitles, language, tokenizer)
    )
...
def cutAndTokenizeText():
    ...
    return json.dumps(
        tokenizerService.cutAndTokenizePlainText(text, language, chunkSize)
    )
...
def cutSubtitlesIntoChunks():
    ...
    return json.dumps(
        tokenizerService.cutSubtitlesIntoChunks(subtitles, language, chunkSize)
    )
...
def getSubtitleContent():
    ...
    try:
        ...
        return HTTPResponse(
            status=415,
            body=f'Error: the file {fileName} was not identified as a supported subtitle file.',
        )
    except UnicodeDecodeError:
        return HTTPResponse(
            status=415,
            body=f'Error: this file {fileName} could not be decoded properly. Confirm the file is Unicode',
        )
    except (IOError, Pysubs2Error):
        return HTTPResponse(
            status=415, body=f'Error: the file {fileName} could not be read.'
        )
...
def install_language_model():
    ...
    if packageManagerService.install_language_model(language, tokenizer):
        return HTTPResponse(
            status=200, body='Language and dependencies installed correctly'
        )
    else:
